...
class Auctions_cdb extends Auctions {
	public function create($prm=[],$data=false){
		...
		foreach($data['data']['tender']['lots'] as &$l){
			...
			if(!isset($dataAuction['source']['data']['tender']['lot']['value']['currency'])) $dataAuction['source']['data']['tender']['lot']['value']['currency']=isset($data['data']['tender']['value']['currency'])?$data['data']['tender']['value']['currency']:'UNKNOWN';
